...
/*    private fun saveDataLocally(record: RecordModel, shifts: List<TimeRange>, user: UserModel): Boolean {
        ...
        // ✅ Use record.localTime (aligned 5-min time) instead of current system time
        val localTimeFormatter = SimpleDateFormat("HH:mm:ss", Locale.getDefault())
        val currentCal = Calendar.getInstance().apply {
            val parsed = localTimeFormatter.parse(record.localTime)
            if (parsed != null) time = parsed
            set(Calendar.SECOND, 0)
            set(Calendar.MILLISECOND, 0)
        }
        val currentTimeStr = record.localTime
        var insideWindow = false
